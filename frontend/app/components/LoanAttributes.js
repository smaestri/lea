	...
	render() {
		...
		return (
			<ul className="pin">
				...
				<li>Demande effectuée le {formatDate(this.props.loan.dateDemande)}
					par {this.props.loan.emprunteur.fullName}</li>
				{this.props.loan.dateAccept &&
				<li>Demande acceptée le {formatDate(this.props.loan.dateAccept)}
					par {this.props.loan.preteur.fullName}</li>}
				{this.props.loan.dateEnvoi &&
				<li>Livre renvoyé le {formatDate(this.props.loan.dateEnvoi)}
					par {this.props.loan.emprunteur.fullName}</li>}
				{displayTextCurrent && <li>Livre en cours de lecture par l'emprunteur </li>}
				{this.props.isHistory && this.props.loan.motifRefus &&
				<li>Prêt refusé le {formatDate(this.props.loan.dateRefus)}
					par {this.props.loan.preteur.fullName} avec le
					motif: {this.props.loan.motifRefus}</li>}
				{this.props.isHistory && !this.props.loan.motifRefus &&
				<li>Prêt clos le {formatDate(this.props.loan.dateCloture)}
					par {this.props.loan.preteur.fullName}</li>}
				{displaySendText &&
				<li>Vous êtes en train de lire ce livre. Renvoyer le à son propriétaire une fois
					terminé!</li>}
				{this.props.displayLinks && !this.props.isHistory && <Link
					to={'loan-detail/' + this.props.loan.id + '/' + isLending}>{envoyerMessage}</Link>}
			</ul>
		)
	}
}
